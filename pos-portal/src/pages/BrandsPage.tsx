  ...
  return (
    <div className="p-4 sm:p-6">
      <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
        <h1 className="text-xl font-semibold text-gray-900 dark:text-gray-100">Brands</h1>
        <button
          onClick={openCreate}
        ...
        <div className="zr-table-wrap">
          <table className="zr-table min-w-[600px]">
            ...
            <tbody>
              {filtered.map((b) => (
                <tr key={b.id}>
                  <td><EntityIdChip id={b.id} ref={b.ref} /></td>
                  <td className="font-medium">
                    <Link to={`/brands/${b.id}`} className="hover:text-brand-600 transition-colors">
                      {b.name}
                    </Link>
                  </td>
                  <td><EntityIdChip id={b.group_id} ref={groups.find((g) => g.id === b.group_id)?.ref} /></td>
                  <td className="text-[var(--zr-muted)]">{groupName(b.group_id)}</td>
                  <td>
        ...
        >
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {!editing && (
              <>
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Brand email</label>
                  <input
                    type="email"
                    value={masterEmail}
