...
export function InvoiceDetailPage() {
  ...
  if (isLoading) return <div className="p-4 sm:p-6 text-sm text-gray-400 dark:text-gray-500">Loading…</div>
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6">
        ...
        <table className="w-full text-sm min-w-[560px]">
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
            {invoice.line_items.map((item) => (
              <Fragment key={item.id}>
                <tr className="hover:bg-gray-50 dark:hover:bg-gray-800/60">
                  ...
                  <td className="px-4 py-3 text-right font-medium text-gray-900 dark:text-gray-100">{centsToDisplay(item.line_total_cents)}</td>
                </tr>
                {item.modifiers.map((mod) => (
                  <tr key={mod.id} className="text-gray-400 dark:text-gray-500 text-xs">
            ...
            {invoice.line_items.length === 0 && (
              <tr>
                <td colSpan={4} className="px-4 py-8 text-center text-gray-400 dark:text-gray-500">No line items.</td>
              </tr>
            )}
          </tbody>
        </table>
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                {invoice.tax_breakdown.map((t) => (
                  <tr key={t.id}>
                    ...
                    <td className="px-4 py-2 text-right text-gray-900 dark:text-gray-100">{centsToDisplay(t.tax_amount_cents)}</td>
                  </tr>
                ))}
                {invoice.tax_breakdown.length === 0 && (
                  <tr><td className="px-4 py-4 text-center text-gray-400 dark:text-gray-500">No tax applied.</td></tr>
                )}
              </tbody>
            </table>
          </div>
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                {invoice.payments.map((p) => (
                  <tr key={p.id}>
                    ...
                    <td className="px-4 py-2 text-gray-400 dark:text-gray-500 text-xs">{p.reference ?? ''}</td>
                    <td className="px-4 py-2 text-right text-gray-900 dark:text-gray-100">{centsToDisplay(p.amount_cents)}</td>
                  </tr>
                ))}
                {invoice.payments.length === 0 && (
                  <tr><td className="px-4 py-4 text-center text-gray-400 dark:text-gray-500">No payments recorded.</td></tr>
                )}
              </tbody>
            </table>
          </div>
      ...
      <div className="w-full sm:w-64 sm:ml-auto space-y-1 text-sm">
        <div className="flex justify-between"><span className="text-gray-500 dark:text-gray-400">Subtotal</span><span className="text-gray-900 dark:text-gray-100">{centsToDisplay(invoice.subtotal_cents)}</span></div>
        <div className="flex justify-between"><span className="text-gray-500 dark:text-gray-400">Tax</span><span className="text-gray-900 dark:text-gray-100">{centsToDisplay(invoice.tax_cents)}</span></div>
        {invoice.discount_cents > 0 && (
          <div className="flex justify-between">
            <span className="text-gray-500 dark:text-gray-400">Discount{invoice.discount_reason ? ` (${invoice.discount_reason})` : ''}</span>
            <span className="text-gray-900 dark:text-gray-100">-{centsToDisplay(invoice.discount_cents)}</span>
          </div>
        )}
        <div className="flex justify-between font-semibold border-t border-gray-200 dark:border-gray-700 pt-1"><span>Total</span><span>{centsToDisplay(invoice.total_cents)}</span></div>
      </div>
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
              {changeLog.map((entry) => (
                <tr key={entry.id}>
                  ...
                  <td className="px-4 py-3 text-gray-900 dark:text-gray-100">{ACTION_LABELS[entry.action] ?? entry.action}</td>
                  <td className="px-4 py-3 text-gray-700 dark:text-gray-300">{entry.actor_name ?? entry.actor_email ?? (entry.actor_type === 'system' ? 'System' : 'Unknown')}</td>
                </tr>
              ))}
              {changeLog.length === 0 && (
                <tr><td colSpan={3} className="px-4 py-8 text-center text-gray-400 dark:text-gray-500">No changes recorded.</td></tr>
              )}
            </tbody>
          </table>
        </div>
